...
def convert_usd_to_inr(usd_range: str) -> str:
    ...
    if len(numbers) >= 2:
        try:
            ...
            # Convert to INR (keep as integers to avoid float formatting issues)
            inr_min = int(round(usd_min * USD_TO_INR))
            inr_max = int(round(usd_max * USD_TO_INR))

            # Format as Indian currency (with lakhs format)
            def format_inr(amount: int) -> str:
                if amount >= 10_000_000:  # 1 crore or more
                    crores = amount // 10_000_000
                    lakhs = (amount % 10_000_000) // 100_000
                    if lakhs > 0:
                        return f"₹{crores}.{lakhs:02d} crores"
                    return f"₹{crores} crores"

                if amount >= 100_000:  # 1 lakh or more
                    lakhs = amount // 100_000
                    thousands = (amount % 100_000) // 1000
                    if thousands > 0:
...
class AIService:
    ...
    def _init_google_genai(self) -> bool:
        ...
        try:
            ...
            if not api_key or api_key == 'your_actual_gemini_api_key_here' or api_key == 'your_google_gemini_api_key_here':
            ...
            self.google_genai_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={api_key}"
            
            if GOOGLE_GENAI_AVAILABLE:
                # Also set up the SDK if available
                genai_module.configure(api_key=api_key)
                self.genai_model = genai_module.GenerativeModel('gemini-2.0-flash')
    ...
    def _generate_with_fallback_ai(self, prompt: str) -> str:
        ...
        # Prefer Ollama when enabled (unit tests expect only a single POST call).
        ...
        # Try Google Generative AI next (Gemini) - Primary service
